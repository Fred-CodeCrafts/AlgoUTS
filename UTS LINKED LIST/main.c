...
{
    char singer[80];
    printf("Enter the singer's name to search for: ");
    fflush(stdin);
    gets(singer);
...
void albumfav() {
    char singer[80];
    printf("Enter the singer's name of the song you want to copy to favorite: ");
    fflush(stdin);
    gets(singer);
    ...
    while (!feof(fpPlaylist)) {
        ...
        if (strcmp(fPenyanyi, singer) == 0) {
            ...
            printf("Song copied to favorites: %s - %s\n", fPenyanyi, fJudul);
        }
    }
    fclose(fpPlaylist);
    fclose(fpFav);
